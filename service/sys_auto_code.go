...
import (
	...
	"go-admin/global"
	"go-admin/model"
	"go-admin/model/resource/autocode_template/subcontract"
	"go-admin/utils"
...
func makeDictTypes(autoCode *model.AutoCodeStruct) {
...
func CreateTemp(autoCode model.AutoCodeStruct, ids ...uint) (err error) {
	...
	// 增加判断: 重复创建struct
	if autoCode.AutoMoveFile && Repeat(autoCode.BusinessDB, autoCode.StructName, autoCode.Package) {
		return RepeatErr
	}
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		var routePath = filepath.Join(global.GVA_CONFIG.AutoCode.Root,
			global.GVA_CONFIG.AutoCode.Server, global.GVA_CONFIG.AutoCode.SInitialize, "router.go")
		var imporStr = fmt.Sprintf("go-admin//model/%s", autoCode.Package)
...
func DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
...
func getNeedList(autoCode *model.AutoCodeStruct) (dataList []tplData, fileList []string, needMkdir []string, err error) {
	...
	tplFileList, err := GetAllTplFile(autocodePath, nil)
	if err != nil {
...
func CreateAutoCode(s *model.SysAutoCode) error {
	if s.PackageName == "autocode" || s.PackageName == "system" || s.PackageName == "example" || s.PackageName == "" {
		return errors.New("不能使用已保留的package name")
	}
	if !errors.Is(global.GVA_DB.Where("package_name = ?", s.PackageName).First(&model.SysAutoCode{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同PackageName")
	}
	if e := CreatePackageTemp(s.PackageName); e != nil {
		return e
	}
	return global.GVA_DB.Create(&s).Error
}

func GetPackage() (pkgList []model.SysAutoCode, err error) {
	err = global.GVA_DB.Find(&pkgList).Error
	return pkgList, err
}

func DelPackage(a model.SysAutoCode) error {
	return global.GVA_DB.Delete(&a).Error
}

func CreatePackageTemp(packageName string) error {
	Init(packageName)
...
// CreatePlug 自动创建插件模板
func CreatePlug(plug model.AutoPlugReq) error {
	// 检查列表参数是否有效
	plug.CheckList()
	tplFileList, _ := GetAllTplFile(plugPath, nil)
...
func InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
...
func installation(path string, formPath string, toPath string) error {
	...
	if err == nil {
		//.L().Error("autoPath 已存在同名插件，请自行手动安装", zap.String("to", to))
...
//	"github.com/otiai10/copy" 包更改了型参
//
